...
from sql import get_user_by_phone, get_farm
...
def get_db_connection():
    return pymssql.connect(
        ...
        as_dict=True  # ✅ Returns results as dictionaries instead of tuples
    )
def save_or_update_transcript(caller_id, role, message, farmer_id="10667", token_id="0"):
    ...
    try:
        ...
        # Find existing document for this caller
        existing_doc = call_transcript_collection.find_one({"caller_id": caller_id})

        if existing_doc:
            # Count existing messages to determine next message field
            message_count = 0
            while f"message_{message_count + 1}" in existing_doc:
                message_count += 1

            # Add new message and role
...
def extract_caller_id_from_stream_sid(stream_sid):
    ...
    # Extract a more meaningful caller ID from stream_sid
    # Format: keep only alphanumeric characters and limit length
    if stream_sid:
        # Remove common prefixes and clean up
        caller_id = stream_sid.replace('MZ', '').replace('CA', '').replace('_', '')
        # Take first 10 characters for readability
...
def detect_language_mix(text):
    ...
    # Common Hindi/Marathi words and patterns
...
def get_weather_data(farm_id):
    ...
    try:
        with get_db_connection() as cnxn:
            cursor = cnxn.cursor()
            query = """
            SELECT TOP 7 ClimateDate, MinTemp, MaxTemp, Rainfall, Humidity, 
                   CloudsCover, CloudStatus, Windspeed
...
@app.get("/conversation/{caller_id}")
async def get_conversation_history(caller_id: str):
    """Get conversation history for a specific caller"""
    try:
        conversation = call_transcript_collection.find_one({"caller_id": caller_id})
        if conversation:
            # Convert ObjectId to string for JSON serialization
            conversation['_id'] = str(conversation['_id'])
            return JSONResponse(content=conversation)
        else:
            return JSONResponse(content={"message": "No conversation found for this caller"}, status_code=404)
    except Exception as e:
...
async def get_all_conversations():
    ...
    try:
        conversations = list(call_transcript_collection.find({}, {"caller_id": 1, "farmer_id": 1, "created_at": 1}))
...
#     """Handle incoming calls and connect to voice assistant."""
#
#     # Extract caller phone number from Plivo webhook
...
#
#     # Add greeting message (Plivo uses Speak element instead of Say)
#     speak_element = plivoxml.SpeakElement(
#         "नमस्कार! Welcome to Agripilot! आपका AI farming assistant connect हो रहा है आपके farm के latest data के साथ। आप अपने crops, soil, weather या government schemes के बारे में पूछ सकते हैं।"
#     )
#     response.add(speak_element)
#
#     # Add pause
#     wait_element = plivoxml.WaitElement(length=1)
#     response.add(wait_element)
#
#     # Add second message
#     speak_element2 = plivoxml.SpeakElement("आप अपना सवाल पूछ सकते हैं! Go ahead!")
#     response.add(speak_element2)
#
#     # Get host for WebSocket URL
#     host = request.url.hostname
#
#     # Create Stream element for Plivo (equivalent to Twilio's Connect)
#     stream_element = plivoxml.StreamElement(
#         f'wss://{host}/media-stream',
#         bidirectional=True,
#         audioTrack="both",
#         streamTimeout=86400  # 24 hours timeout
#     )
#     response.add(stream_element)
#
...
async def handle_incoming_call(request: Request):
    ...
    try:
        ...
        # Start background farm data processing
        ...
        # Store user_details in a global variable or session for websocket access
...
def save_conversation_transcript(call_details, role, message):
    ...
    try:
        ...
        existing_conversation = call_transcript_collection.find_one({
        ...
        if existing_conversation:
            ...
            result = call_transcript_collection.insert_one(conversation_doc)
                        ...
                        if data['event'] == 'media' and openai_ws and openai_ws.open:
                            ...
                            try:
                                ...
                                response = requests.post(
                                    ...
                                    json={"farmerMasterLoginId": current_user_details.get('MasterLoginId')},
                    ...
                    async for openai_message in openai_ws:
                        ...
                        if response.get('type') == 'conversation.item.input_audio_transcription.completed':
                            ...
                            if transcript and call_details:
                                ...
                                save_conversation_transcript(call_details, "user", transcript)
                        ...
                        if response.get('type') == 'response.audio_transcript.done':
                            ...
                            if transcript and call_details:
                                ...
                                save_conversation_transcript(call_details, "assistant", transcript)
                                current_assistant_response = ""
...
# @app.websocket("/media-stream")
# async def handle_media_stream(websocket: WebSocket):
#     """Main WebSocket handler for Plivo-OpenAI communication with improved interruption handling."""
#     await websocket.accept()
#
...
#
#     # Enhanced interruption tracking with immediate response capability
#     assistant_state = {
#         'is_speaking': False,
...
#         'interrupt_in_progress': False,  # New flag to prevent race conditions
...
#     call_details = {}
#
#     # Interruption management with faster response
#     interruption_lock = asyncio.Lock()
#     pending_user_audio = False
#     user_speaking_detected = False  # Track if user is actively speaking
#
#     async def call_disconnect_api():
#         """Call the disconnect API when the call ends."""
#         nonlocal disconnect_called, connection_active
...
#         if call_id in active_call_sessions:
#             del active_call_sessions[call_id]
#
#     async def immediate_audio_stop():
#         """Immediately stop all audio playback - called as soon as user speaks"""
#         try:
#             if websocket.client_state == WebSocketState.CONNECTED and stream_id:
#                 # Send multiple immediate clear commands
#                 clear_commands = [
#                     {"event": "clear", "streamId": stream_id},
...
#
#                 print("🛑 IMMEDIATE STOP: All Plivo audio cleared instantly")
#                 return True
#         except Exception as e:
#             print(f"Error in immediate audio stop: {e}")
#             return False
#
#     async def handle_interruption():
#         """Ultra-fast interruption handling with immediate audio stopping."""
#         if assistant_state['interrupt_in_progress']:
#             return  # Prevent multiple simultaneous interruptions
#
#         assistant_state['interrupt_in_progress'] = True
#
#         try:
#             print("⚡ ULTRA-FAST INTERRUPTION: Immediate stop initiated")
#
#             # STEP 1: INSTANT AUDIO TERMINATION (No waiting, no locks)
#             stop_success = await immediate_audio_stop()
#
#             # STEP 2: CANCEL OPENAI RESPONSE IMMEDIATELY (Parallel to audio stop)
#             openai_tasks = []
#             if openai_ws and openai_ws.open:
#                 if assistant_state['current_response_id']:
#                     cancel_task = asyncio.create_task(openai_ws.send(json.dumps({
#                         "type": "response.cancel"
#                     })))
#                     openai_tasks.append(cancel_task)
#                     print(f"⚡ INSTANT CANCEL: OpenAI response {assistant_state['current_response_id']}")
#
#                 # STEP 3: CLEAR INPUT BUFFER IMMEDIATELY
#                 clear_task = asyncio.create_task(openai_ws.send(json.dumps({
#                     "type": "input_audio_buffer.clear"
#                 })))
#                 openai_tasks.append(clear_task)
#
#                 # Execute all OpenAI commands in parallel
#                 if openai_tasks:
#                     await asyncio.gather(*openai_tasks, return_exceptions=True)
#                     print("⚡ PARALLEL EXECUTION: All OpenAI stops sent simultaneously")
#
#             # STEP 4: FORCE MULTIPLE AUDIO CLEARS (Ensure complete stop)
#             if stop_success and websocket.client_state == WebSocketState.CONNECTED:
#                 # Send aggressive clear sequence
#                 clear_sequence = []
#                 for i in range(5):  # Send 5 clear commands rapidly
#                     clear_sequence.append(websocket.send_json({
#                         "event": "clear",
#                         "streamId": stream_id
#                     }))
#
#                 await asyncio.gather(*clear_sequence, return_exceptions=True)
#                 print("⚡ AGGRESSIVE CLEAR: 5 simultaneous clear commands sent")
#
#             # STEP 5: IMMEDIATE STATE RESET (No delays)
#             mark_queue.clear()
...
#                 'audio_start_time': None,
#                 'should_interrupt': False,
#                 'last_audio_timestamp': latest_media_timestamp
#             })
#
#             nonlocal current_assistant_response
#             current_assistant_response = ""
#
#             print("⚡ INSTANT RESET: All state cleared immediately")
#
#         except Exception as e:
#             print(f"⚡ INTERRUPTION ERROR: {e}")
#         finally:
#             assistant_state['interrupt_in_progress'] = False
#
#     async def monitor_websocket_connection():
#         """Monitor WebSocket connection and detect disconnections"""
#         try:
...
#                 """Process audio from Plivo and forward to OpenAI."""
#                 nonlocal stream_id, latest_media_timestamp, caller_id, call_details, connection_active
#                 nonlocal pending_user_audio, user_speaking_detected
#
#                 try:
#                     async for message in websocket.iter_text():
#                         if not connection_active:
...
#                             audio_data = data['media']['payload']
#
#                             # Only forward audio if user is not currently being processed for interruption
#                             if not assistant_state['should_interrupt'] and not user_speaking_detected:
#                                 audio_append = {
#                                     "type": "input_audio_buffer.append",
#                                     "audio": audio_data
#                                 }
...
#                                 api_call_id = response_data.get('details', {}).get('callID')
#
#                                 if all([token_id, farmer_id, api_call_id]):
#                                     call_details['token_id'] = token_id
#                                     call_details['farmerMasterLoginId'] = farmer_id
#                                     call_details['callID'] = api_call_id
#                                     print(f"API call successful. Token ID: {token_id}")
#
#                             except Exception as e:
#                                 print(f"API call failed: {e}")
#
...
#
#             async def send_to_plivo():
#                 """Process OpenAI responses and send audio to Plivo with ultra-fast interruption."""
#                 nonlocal stream_id, current_user_transcript, current_assistant_response, caller_id, call_details, connection_active
#                 nonlocal pending_user_audio, user_speaking_detected
#
#                 try:
#                     async for openai_message in openai_ws:
...
#                                 save_conversation_transcript(call_details, "user", transcript)
#
#                         # CRITICAL: INSTANT INTERRUPTION ON SPEECH START
#                         if response.get('type') == 'input_audio_buffer.speech_started':
#                             print("🎤 USER SPEAKING DETECTED - INSTANT INTERRUPTION")
#                             user_speaking_detected = True
#                             pending_user_audio = True
#
#                             # IMMEDIATE interruption - no conditions, no delays
#                             if assistant_state['is_speaking']:
#                                 assistant_state['should_interrupt'] = True
#                                 # Fire interruption immediately in background
#                                 asyncio.create_task(handle_interruption())
#                                 print("⚡ INSTANT TRIGGER: Interruption fired immediately")
#
#                         # Handle speech stopped - prepare for new input
#                         if response.get('type') == 'input_audio_buffer.speech_stopped':
#                             print("🎤 User speech ended - preparing for new response")
#                             user_speaking_detected = False
#
#                             # Brief pause to ensure interruption completes
#                             if assistant_state['should_interrupt']:
#                                 await asyncio.sleep(0.1)  # Minimal wait
#                                 assistant_state['should_interrupt'] = False
#                                 print("🔄 Interrupt flag reset")
#
#                             # Commit user audio for processing
#                             if openai_ws and openai_ws.open:
#                                 await openai_ws.send(json.dumps({
#                                     "type": "input_audio_buffer.commit"
#                                 }))
#                                 print("✅ NEW USER INPUT: Committed for processing")
#
#                             pending_user_audio = False
#
#                         # Handle assistant response creation
#                         if response.get('type') == 'response.created':
#                             # Only allow new responses if no interruption pending
#                             if not assistant_state['should_interrupt'] and not user_speaking_detected:
#                                 current_assistant_response = ""
#                                 assistant_state['is_speaking'] = True
#                                 assistant_state['current_response_id'] = response.get('response', {}).get('id')
#                                 assistant_state['audio_start_time'] = latest_media_timestamp
#                                 print(f"🤖 Assistant response started: {assistant_state['current_response_id']}")
#                             else:
#                                 print("🚫 BLOCKED: Response blocked due to user activity")
#
#                         # Handle assistant text/transcript accumulation
#                         if response.get('type') in ['response.text.delta', 'response.audio_transcript.delta']:
#                             if not assistant_state['should_interrupt'] and not user_speaking_detected:
...
#                             assistant_state['current_response_id'] = None
#                             assistant_state['current_item_id'] = None
#                             assistant_state['audio_start_time'] = None
...
#                                 transcript = response.get('transcript', '').strip()
#                                 if transcript and call_details:
#                                     print(f"Assistant responded: {transcript}")
#                                     save_conversation_transcript(call_details, "assistant", transcript)
#                             current_assistant_response = ""
#
#                         # CRITICAL: Audio streaming with instant interruption check
#                         if response.get('type') == 'response.audio.delta' and 'delta' in response:
#                             try:
#                                 # TRIPLE CHECK before sending any audio
#                                 if (assistant_state['should_interrupt'] or
#                                         user_speaking_detected or
#                                         pending_user_audio or
#                                         assistant_state['interrupt_in_progress']):
#                                     print("🚫 AUDIO BLOCKED: User activity detected")
#                                     continue
#
#                                 if websocket.client_state == WebSocketState.CONNECTED:
#                                     audio_payload = base64.b64encode(base64.b64decode(response['delta'])).decode(
...
#                                     if not user_speaking_detected and not assistant_state['should_interrupt']:
#                                         await websocket.send_json(audio_delta)
#                                         assistant_state['last_audio_timestamp'] = latest_media_timestamp
#
#                                         if response.get('item_id'):
#                                             assistant_state['current_item_id'] = response['item_id']
#
#                                         await send_mark(websocket, stream_id)
#                                     else:
#                                         print("🚫 LAST-SECOND BLOCK: Audio blocked at send time")
#                                 else:
#                                     print("WebSocket disconnected - stopping audio")
#                                     await call_disconnect_api()
#                                     break
#
#                             except Exception as e:
...
#                             print(f"OpenAI API Error: {response.get('error', {})}")
#
#                 except Exception as e:
#                     print(f"Error in OpenAI communication: {e}")
#                     await call_disconnect_api()
